...
import { 
  Governance,
  Dao
} from "./models/sena-tests-governance.ts";

import { 
  StakeRegistry,
  StakePoolDikoV1
} from './models/sena-tests-stake.ts';

import { 
  DikoToken,
  StDikoToken
} from './models/sena-tests-tokens.ts';

import * as Utils from './models/sena-tests-utils.ts'; Utils;
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    let result = governance.createProposal(
      ...
      10, 
      "Test Title",
      "https://discuss.sena.finance/my/very/long/url/path",
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    call = governance.getMemberVoteTokens(1, wallet_1, "sena-token");
    ...
    call = governance.getMemberVoteTokens(1, wallet_2, "sena-token");
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    // Create proposal to start at block 1
    let contractChange1 = Governance.contractChange("governance", Utils.qualifiedName('sena-governance-tv1-1'), true, true);
    let result = governance.createProposal(
      wallet_1, 
      1, 
      "Replace governance",
    ...
    call.result.expectSome().expectPrincipal(Utils.qualifiedName('sena-governance-tv1-1'));

    contractChange1 = Governance.contractChange("oracle", Utils.qualifiedName('new-oracle'), true, true);

    let block = chain.mineBlock([
      Tx.contractCall("sena-governance-tv1-1", "propose", [
        types.principal(Utils.qualifiedName('sena-stake-pool-diko-v1-1')),
        types.uint(1504),
        types.utf8("Replace Oracle"),
        types.utf8("https://discuss.sena.finance/my/very/long/url/path"),        
    ...
    block = chain.mineBlock([
      Tx.contractCall("sena-governance-tv1-1", "vote-for", [
        types.principal(Utils.qualifiedName('sena-stake-pool-diko-v1-1')),
        types.principal(Utils.qualifiedName('sena-token')),
        types.uint(1),
    ...
    // End proposal
    block = chain.mineBlock([
      Tx.contractCall("sena-governance-tv1-1", "end-proposal", [
        types.uint(1)
      ], deployer.address)
    ]);
    block.receipts[0].result.expectOk().expectUint(3200);
    ...
    // Check if proposal updated
    call = chain.callReadOnlyFn("sena-governance-tv1-1", "get-proposal-by-id", [types.uint(1)], deployer.address)
    call.result.expectTuple()["is-open"].expectBool(false);
    ...
    block = chain.mineBlock([
      Tx.contractCall("sena-governance-v2-1", "propose", [
        types.principal(Utils.qualifiedName('sena-stake-pool-diko-v1-1')),
        types.uint(2504),
        types.uint(123),
        types.utf8("Replace Oracle"),
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    call.result.expectSome().expectPrincipal(deployer.address);
    call = dao.getQualifiedNameByName("oracle");
    call.result.expectSome().expectPrincipal(Utils.qualifiedName('sena-oracle-v1-1'));
  }
});
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    let result = stakeRegistry.stake(wallet_1, 'sena-stake-pool-diko-v1-1', 'sena-token', 100);
    ...
    result = stakeRegistry.stake(wallet_1, 'sena-stake-pool-diko-v1-1', 'sena-token', 100);
    ...
    // Vote with DIKO
    result = governance.voteForProposal(wallet_1, 1, 10, "sena-token");
    result.expectOk().expectUint(3200);
    result = governance.voteAgainstProposal(wallet_1, 1, 1, "sena-token");
    result.expectOk().expectUint(3200);
    ...
    result = governance.returnVotes(1, wallet_1, "sena-token");
    result.expectOk();
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    result = governance.voteForProposal(wallet_1, 1, 10, "sena-token");
    result.expectOk().expectUint(3200);
    result = governance.voteForProposal(wallet_2, 1, 100, "sena-token");
    result.expectOk().expectUint(3200);

    // Tokens used to vote
    let call:any = governance.getMemberVoteTokens(1, wallet_1, "sena-token");
    call.result.expectTuple()["amount"].expectUintWithDecimals(10);
    ...
    let block = chain.mineBlock([
      Tx.contractCall("sena-governance-v2-1", "return-votes-to-member", [
        types.principal(Utils.qualifiedName("sena-token")),
        types.uint(1),
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    // New proposal
    let contractChange = Governance.contractChange("governance", Utils.qualifiedName('sena-governance-tv1-1'), false, false);
    let result = governance.createProposal(
      wallet_1, 
      1, 
      "Test Title",
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    call = dikoToken.balanceOf(Utils.qualifiedName('sena-diko-init'));
    ...
    let block = chain.mineBlock([
      Tx.contractCall("sena-token", "burn", [
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    let block = chain.mineBlock([
      Tx.contractCall("sena-governance-v2-1", "toggle-governance-shutdown", [], wallet_1.address)
    ]);
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    let block = chain.mineBlock([
      Tx.contractCall("sena-governance-v2-1", "add-contract-address", [
